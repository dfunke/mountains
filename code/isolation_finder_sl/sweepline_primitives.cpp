...
SlEvent::SlEvent(Elevation elev, SlEventType type, LatLng latLng) : LatLng(latLng) {
  mElev = elev;
  mType = type;
  mOffsets = Offsets(0,0);
}

void SlEvent::initialize(Elevation elev, SlEventType type, LatLng latLng, Offsets offsets) {
  mElev = elev;
  mType = type;
  mOffsets = offsets;
  updateLatLng(latLng);

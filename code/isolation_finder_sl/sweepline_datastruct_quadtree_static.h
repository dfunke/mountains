...
template <std::size_t N> struct Leaf {
  SlEvent *events[N];
  uint16 activeCounter = 0;
};

struct Quad {
  float mMaxLat, mMinLat, mMaxLng, mMinLng, minDistanceToPoint;
  int idx, counterIdx;
  void set(int idx, int counterIdx, float minLat, float maxLat, float minLng,
           float maxLng, float minDistanceToPoint) {
    this->idx = idx;
...
public:
  ...
  explicit SweeplineDatastructQuadtreeStatic(
      float latMin, float latMax, float lngMin, float lngMax,
      std::size_t maxPoints,
      std::function<Offsets(float lat, float lng)> toOffsets) {
  ...
  void remove(const SlEvent *n) override {
    ...
    for (uint16 i = 0; i < mCells[index].activeCounter; i++) {
      if (mCells[index].events[i] != nullptr &&
          mCells[index].events[i]->latitude() == n->latitude() &&
          mCells[index].events[i]->longitude() == n->longitude()) {
         mCells[index].events[i] = nullptr;
      }
    }
  }
  ...
  void insert(SlEvent *n) override {
    ...
    //uint16 activeCounter = mCells[index].activeCounter;
  ...
  IsolationRecord calcPeak(const SlEvent *node) override {
    ...
    Quad root;
    root.set(0, 0, mMinLat, mMaxLat, mMinLng, mMaxLng,
             0); // min distance here not important!!
    findNearest(node, root, 0, mCellsSize, (mCellsSize - 1) / 3, &ir);
    ir.distance = ir.closestHigherGround.distanceEllipsoid(*node);
    return ir;
  }

  void saveAsImage(int w, int h, int level, Offsets peak,
                   Offsets highestElev) override {
    FILE *imageFile;
    ...
    for (int i = 0; i < mCellsSize; ++i) {
      for (uint j = 0; j < mCells[i].activeCounter && j < LeafSize; ++j) {
        Offsets o = mToOffsets(mCells[i].events[j]->latitude(),
                               mCells[i].events[j]->longitude());
    ...
    float len = std::max(std::abs(peak.x() - highestElev.x()),
                         std::abs(peak.y() - highestElev.y()));
    for (int i = 0; i < len; i++) {
      float t = ((float)i) / len;
  ...
  void saveHeatMap(int level) override {
    ...
    for (int i = 0; i < cellWIdth; ++i) {
      for (int j = 0; j < cellWIdth; ++j) {
        int idx = GetIndexForLatLng(mMinLat + ((i + 0.f) / cellWIdth),
                                    mMinLng + ((j + 0.f) / cellWIdth));
...
private:
  void basicSetup(float latMin, float latMax, float lngMin, float lngMax,
                  std::size_t maxPoints) {
    mMinLat = latMin;
    mMaxLat = latMax;
    mMinLng = lngMin;
    ...
    mNodeCounts = new uint32[(mCellsSize - 1) / 3];
  ...
  void findNearest(const SlEvent *n, const Quad p, const int level,
                   const int bucketSize, const int counterSize,
                   IsolationRecord *ir) {
    if (level == mMaxLevel) {
      uint16 cellSize = mCells[p.idx].activeCounter;
      ...
      for (uint i = 0; i < cellSize; ++i) {
        ...
        if (toCheck == nullptr || toCheck->equal(*n)) {
          continue;
        }
    ...
    // std::cout << "Size of this cell " << mCellSizes[p.counterIdx] <<
    // std::endl;
    //  Create 4 Quads
    ...
    for (int i = 0; i < 4; ++i) {
      if ((nCunterSize > 0 &&
           mNodeCounts[p.counterIdx + 1 + i * nCunterSize] > 0) ||
          (nCunterSize == 0 && mCells[p.idx + i * nBucket].activeCounter > 0)) {
        ...
        const float minDistance = shortestDistanceToQuadrillateral(
            n, LatLng(maxLat, maxLng), LatLng(minLat, minLng));
        quads[quadsIdx].set(p.idx + i * nBucket,
                            p.counterIdx + 1 + i * nCunterSize, minLat, maxLat,
                            minLng, maxLng, minDistance);
        ++quadsIdx;
      }
    }
    assert(quadsIdx > 0);
    ...
    for (int i = 0; i < quadsIdx; ++i) {
      if (!ir->foundHigherGround ||
          quads[i].minDistanceToPoint < ir->distance) {
